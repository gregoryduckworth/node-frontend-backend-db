...
describe('Auth Endpoints', () => {
  ...
  describe('POST /auth/register', () => {
    ...
    it('should fail if email already exists', async () => {
      ...
      const res = await request(app).post(registerEndpoint).send(validRegisterData);
  ...
  describe('POST /auth/forgot-password', () => {
    ...
    it("should return 200 even if email doesn't exist (security)", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.message).toBe('Password reset instructions sent to your email');
  ...
  describe('POST /auth/reset-password', () => {
    ...
    it('should fail with invalid token', async () => {
      ...
      const res = await request(app).post(resetPasswordEndpoint).send(validResetData);
